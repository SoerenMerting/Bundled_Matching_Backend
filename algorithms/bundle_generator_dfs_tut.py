...
class BundleGeneratorDFSTut(object):
    """This class generates valid bundles for tutor matching via depth-first-search algorithm"""
    def __init__(self,days_priority_dict, ts_dict  ,tutorials_list ,relevant_timeslots, courses_list = list(), max_courses_dict=list(),gap_time=0,lunch_time=0, MAX=100, sorted_dfs=True,randomized = False):
    ...
    def dfs(self,course, current_bundle,bundle_size=0):
        """Search recursively for valid bundles.
        A bundle will be added to self.__solution_list
        if it is complete and valid.
        """
        bs = bundle_size + 1
        next_element = self.__tutorials[bs] if bs < len(self.__tutorials) else None
        ...
        for ts in self.__course_to_timeslots[course]:
            ...
            if (bs > len(self.__tutorials)-1) and valid and length<self.__MAX:
                self.__solution_list.append(new_bundle)
            elif valid and length<self.__MAX:
                self.dfs(next_element,new_bundle,bs)
    ...
    def get_bundles_as_tuples(self):
        ...
        #if self.__sorted:
            #sr = ScoringRule(solution,self.__max_courses_dict,b_l_t)
    ...
    def is_valid(self, current_bundle, timeslot):
        ...
        or does not violate max_course restrictions and does not collide with existing elements in the bundle
        """
